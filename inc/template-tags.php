...
if (!function_exists('outbuilt_post_meta')) :
    ...
    function outbuilt_post_meta($category = true) {
        ...
        // Recipe integration
        $recipe = outbuilt_recipe_integration('wpzoom-recipe-card/block-recipe-card');
        ...
        if ($recipe) {
            $difficulty = $recipe['attrs']['difficulty'][0];
            $time       = $recipe['attrs']['details'][2]['value'];
            $unit       = $recipe['attrs']['details'][2]['unit'];
        }

        $meta_one = $time ? $time . ' ' . $unit : $posted_on;
        $meta_two = $difficulty ? $difficulty : $categories;

        if ($category == true) :
            echo '<span class="byline"> ' . $byline . '</span><span class="posted-on">' . $meta_one . '</span><span class="cat-links">' . $meta_two . '</span>'; // WPCS: XSS OK.
        else :
            echo '<span class="byline"> ' . $byline . '</span><span class="posted-on">' . $meta_one . '</span>'; // WPCS: XSS OK.
        endif;
    }
endif;
...
/**
 * WPZoom Recipe plugin integration
 */
function outbuilt_recipe_integration($block_name) {
